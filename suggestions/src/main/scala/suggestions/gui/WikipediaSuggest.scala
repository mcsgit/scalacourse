...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    def searchTermToSuggestion(term:String):Observable[List[String]]={
      val r=ObservableEx(Search.wikipediaSuggestion(term)).timedOut(5)
      println("searchTermToSuggestion"+term)
      r
    }
    
    // TO IMPLEMENT
    val suggestions: Observable[Try[List[String]]] = {
      val r=searchTerms.concatRecovered(searchTermToSuggestion)
      println("suggestions")
      r
    }
    ...
    val suggestionSubscription: Subscription =  suggestions.observeOn(eventScheduler) subscribe {
      x => { x match{
        ...
        case Failure(e) => {status.text_=("failure in suggestionSubscription")}
      }
      }
    }
    ...
    val selections: Observable[String] = Observable((observer:Observer[String])=>{
			...
			(t: Button) => { if(!suggestionList.selection.items.isEmpty) observer.onNext(suggestionList.selection.items.head) },
    ...
    def suggestionToPage(term:String):Observable[String]={
      val r = ObservableEx(Search.wikipediaPage(term)).timedOut(7)
      println("suggestionToPage:"+term)
      r
    }
    
    
    // TO IMPLEMENT
    val pages: Observable[Try[String]] = selections.concatRecovered(suggestionToPage)
    

    // TO IMPLEMENT
    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe {
      x => { x match{
        case Success(l) =>{editorpane.text_=(l)}
        case Failure(e) => {status.text_=("failure in pageSubscription")}
      }
      }
    }
