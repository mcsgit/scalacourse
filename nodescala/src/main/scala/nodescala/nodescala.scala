...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    var l=createListener(relativePath)
    val subscription1 =l.start
...
//	        println(request+" -> "+((new java.util.Date()).getTime() - now))
	        val t=respond(exchange, ct, handler(request))
	        l=createListener(relativePath)
	        l.start
//	        println("for response -> "+((new java.util.Date()).getTime() - now))
        }
      }
      
    }
    val subscription2 = Future.run()(f)
    subscription2
  }
...
object NodeScala {
  ...
  trait Listener {
    ...
    def nextRequest(): Future[(Request, Exchange)] = {
      ...
      def h(e: Exchange):Unit={
        p.complete(Success(e.request, e))
        removeContext
      }
      p.future onComplete {
        case Success(x) => removeContext
        case Failure(e) => removeContext
      } 
      createContext(h)
      p.future
    }
