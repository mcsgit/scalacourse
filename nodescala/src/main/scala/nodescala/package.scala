...
package object nodescala {
  ...
  implicit class FutureCompanionOps[T](val f: Future.type) extends AnyVal {
    ...
    def never[T]: Future[T] = {
      future{ while(true){ Thread.sleep(1000)}; throw new TimeoutException}
    }
...
//      val p=Future()
//      p onComplete{
//        case Success(p) => Thread.sleep(t.toMillis)
//        case Failure(p) => Thread.sleep(t.toMillis)
//      }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def continue[S](cont: Try[T] => S): Future[S] = {
      continueWithImpl[T,S](f, cont)
    }
        
  }

    def continueWithImpl[T,S](f: Future[T],cont: Try[T] => S): Future[S] = {
     val z=	async{
         scala.async.Async.await(f)
         cont(f.value.get)
         }
        z
    }
  ...
  object CancellationTokenSource {
    ...
    def apply(): CancellationTokenSource = new CancellationTokenSource {
      ...
      def cancellationToken() = new CancellationToken{
    	...
    	def unsubscribe(): Unit={ p.success("unsubscribed")}
      }
    }
