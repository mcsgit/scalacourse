...
class NodeScalaSuite extends FunSuite {
  ...
  test("testing all") {
    val g = Future.all(List(Future { Thread.sleep(1000) ; 1 }, Future { Thread.sleep(1500) ; 2 }))

      assert(Await.result(g, 2.5 second) === List(1,2))
  }
  
  test("testing any") {
    val g = Future.any(List(Future { Thread.sleep(1000) ; 1 }, Future { Thread.sleep(2500) ; 2 }, Future { Thread.sleep(3500) ; throw new Exception }))

      assert(Await.result(g, 2 seconds) == 1)
  }
