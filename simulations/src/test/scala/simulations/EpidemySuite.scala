...
class EpidemySuite extends FunSuite {
  test("An infected person should become sick in 6 days"){
	  var sickTimes = 0
	  for(i <- 0 to 100){
			  val es = new EpidemySimulator
			 for(p<-es.persons) p.infected=true
	
	      while(es.agenda.head.time < 10) es.next
	
	      sickTimes = es.persons.filter(p => p.sick).length
	      assert(sickTimes == es.persons.length, "An infected person should become sick in 6 days")
  ...
  test("An immune person should not become sick "){
	  ...
	  for(i <- 0 to 100){
	      ...
	      while(es.agenda.head.time < 20) es.next
  ...
  test("An infected person should have a probability to die"){
	  var deadTimes = 0
	  for(i <- 0 to 100){
			  val es = new EpidemySimulator
			 for(p<-es.persons) p.infected=true
	
	      while(es.agenda.head.time < 50) es.next
	
	      deadTimes = deadTimes + (if(es.persons.filter(p =>p.dead).isEmpty) 0 else 1)
	  }
	  assert(deadTimes > 0, "An infected person should have a probability to die")
  }
